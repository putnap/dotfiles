...
local workspaces_output =
	execute_command(query_workspaces):gsub('"([a-z-]*)" : ', '["%1"]='):gsub("^%s*%[", "{"):gsub("%]%s*$", "}")
local t = load("return " .. workspaces_output)()
for _, entry in ipairs(t) do
	local workspace_index = entry.workspace
	local monitor_id = math.floor(entry["monitor-appkit-nsscreen-screens-id"])

	local workspace = sbar.add("space", "space." .. workspace_index, {
	...
	workspace:subscribe("aerospace_workspace_change", function(env)
		...
		sbar.animate("tanh", 10, function()
			workspace:set({
				...
				background = {
					border_color = is_focused and colors.green or colors.bg2,
				},
				blur_radius = 30,
			})
		end)
...
sbar.exec("aerospace list-workspaces --focused", function(focused_workspace)
	focused_workspace_index = focused_workspace:match("^%s*(.-)%s*$")
